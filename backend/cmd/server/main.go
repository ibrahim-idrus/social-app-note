...
func main() {
	...
	defer db.Close()
	if err := store.ConfigureInstagramIntegration(context.Background(), db, cfg.InstagramDedicatedAccountID, cfg.InstagramDedicatedUsername, cfg.InstagramAccessToken); err != nil {
		log.Fatal(err)
	}
