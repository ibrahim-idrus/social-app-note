<script lang="ts">
	...
	function remove() { if (note) { deleteNote(note.id); goto(resolve('/notes')); } }
</script>
...
<div class="page page-narrow">
	...
	{#if note}<header class="page-header" style="margin-top:24px"><div><span class="eyebrow">{note.source === 'instagram' ? 'Instagram note' : 'Manual note'}</span><h1>{note.title}</h1><div class="detail-meta"><span class="source">{#if note.source === 'instagram'}<MessageCircle size={13} />{/if}{note.source}</span><span>Created {formatDate(note.createdAt)}</span><span>Updated {formatDate(note.updatedAt)}</span></div></div><div style="display:flex;gap:9px"><Button href={resolve('/notes/[id]/edit', { id: note.id })} variant="outline"><Pencil />Edit</Button><Button variant="outline" aria-label="Delete note" onclick={() => confirmOpen = true}><Trash2 />Delete</Button></div></header>
		<article class="panel detail-body prose">{@html renderMarkdown(note.content)}</article>
	{:else}<section class="panel state-box"><div><h2>Note not found</h2><p>This note may have been deleted or is no longer available.</p><Button href={resolve('/notes')}>Return to notes</Button></div></section>{/if}
</div>
