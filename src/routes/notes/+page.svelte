...
<div class="page">
	...
	<section class="panel" aria-live="polite">
		...
		{:else}{#each visible as note}<a class="note-row" href={resolve('/notes/[id]', { id: note.id })}><div><h3>{note.title}</h3><p>{note.content.replace(/[#*_>`-]/g, '')}</p></div><div class="note-meta"><span class="source">{#if note.source === 'instagram'}<MessageCircle size={12} />{/if}{note.source}</span><br />Updated {formatDate(note.updatedAt)}</div></a>{/each}
