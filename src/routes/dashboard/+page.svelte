...
<div class="page">
	<header class="page-header"><div><span class="eyebrow">Workspace</span><h1>Good morning, {appState.name.split(' ')[0]}</h1><p class="subtle">A quick view of what you’ve captured lately.</p></div><Button href={resolve('/notes/new')}><Plus />Create note</Button></header>
	...
	<div class="dashboard-grid">
		<section class="panel"><div class="panel-head"><h2>Recent notes</h2><Button href={resolve('/notes')} variant="ghost" size="sm">View all <ArrowRight /></Button></div>{#each recent as note}<a class="note-row" href={resolve('/notes/[id]', { id: note.id })}><div><h3>{note.title}</h3><p>{note.content.replace(/[#*_>`-]/g, '')}</p></div><div class="note-meta"><span class="source">{note.source === 'instagram' ? 'Instagram' : 'Manual'}</span><br />{formatDate(note.updatedAt)}</div></a>{/each}</section>
		<aside class="panel"><div class="panel-head"><h2>Instagram inbox</h2></div><div class="panel-body">
			...
			<div class="quick-list"><a class="quick-link" href={resolve('/notes/new')}><NotebookPen size={17} />Write a manual note</a><a class="quick-link" href={resolve('/settings')}><MessageCircle size={17} />{appState.instagram ? 'Manage Instagram' : 'Set up Instagram'}</a></div>
		</div></aside>
	</div>
</div>
