...
<div class="app-frame">
	<header class="mobile-bar">
		<a class="brand" href="/dashboard"><span class="brand-mark"><BookOpenText size={18} /></span>NoteDesk</a>
	...
	<aside class:open class="sidebar">
		<a class="brand desktop-brand" href="/dashboard"><span class="brand-mark"><BookOpenText size={18} /></span>NoteDesk</a>
		...
		<div class="sidebar-foot">
			<Button href="/notes/new" class="w-full"><Plus />New note</Button>
			<button class="profile-row" onclick={() => goto('/login')}><span class="avatar">{appState.name.split(' ').map((part: string) => part[0]).join('').slice(0, 2)}</span><span><strong>{appState.name}</strong><small>{appState.email}</small></span><LogOut size={16} /></button>
		</div>
	</aside>
